...
function QDKP2_OD(text, sender)
    ...
    elseif P1 == "?report" or P1 == "?log" then
        ...
        elseif not (QDKP2_IsInGuild(P2) or P2 == "RAID") then
            ...
            if P3 == "current" or P3 == "last" then
                ...
                reportType = QDKP2_LOC_CurrentSession .. " " .. SessName
    ...
    elseif (P1 == "?classdkp" or P1 == "?dkpclass") then
        ...
        if not QDKP2_COD then
            return { "QDKP2 - This feature is disabled." };
        end
        local arg = P2 or QDKP2class[sender]
        if not arg or arg == "--" then
            return { "QDKP2 - You must provide the class to report." };
        end
        local origArg = arg
        local class
        ...
        if table.getn(list) == 0 then
            return { "QDKP2 - No Guild Members for the given class" };
        end
        QDKP2_netSort(list)
        for i = 1, table.getn(list) do
            if i > QDKP2_LOD_MAXLEN then
                break ;
            end
            local name = list[i]
    ...
    elseif (P1 == "?rankdkp" or P1 == "?dkprank") then
        ...
        if not QDKP2_KOD then
            return { "QDKP2 - This feature is disabled." };
        end
        local rank_str = string.sub(text, 10)
        local rank
        if string.len(rank_str) > 0 then
            rank = string.lower(rank_str);
        end
        rank = rank or string.lower(QDKP2rank[sender])
        if not rank then
            return { "QDKP2 - You must provide the rank to report." };
        end
        if rank == "external" then
            rank = "*external*";
        end
        ...
        if table.getn(list) == 0 then
            return { "QDKP2 - No Guild Members for the given rank" };
        end
        QDKP2_netSort(list)
        for i = 1, table.getn(list) do
            if i > QDKP2_LOD_MAXLEN then
    ...
    elseif P1 == "?help" or P1 == "?commands" or P1 == "?command" or P1 == "?keyword" or P1 == "?keywords" then
        ...
        if QDKP2_NOD then
            table.insert(output, '"?dkp <name>"');
        end
        if QDKP2_COD then
            table.insert(output, '"?classdkp <class>"');
        end
        if QDKP2_KOD then
            table.insert(output, '"?rankdkp <rank>"');
        end
        if QDKP2_ROD then
        ...
        if table.getn(output) == 1 then
            output = { "QDKP2 - No On Demand enabled keywords." };
        end
        return output
    end
end
