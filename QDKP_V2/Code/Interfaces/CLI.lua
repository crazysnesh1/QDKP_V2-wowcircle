...
function QDKP2_CLI_ProcessCommand(text)
    ...
    elseif W1 == "timer" or W1 == "tim" or W1 == "t" then
        ...
        elseif W2 == "bonus" or W2 == "dkp" or W2 == "setbonus" and tonumber(W3) then
            ...
            QDKP2_CLI_ShowUsage("timer on|off|bonus [<dkp>]");
            return
        end
        return
    ...
    elseif W1 == "showlog" then
        ...
        if W2 == "Raid" then
            QDKP2GUI_Log:ShowRaid()
        elseif QDKP2_IsInGuild(W2) then
            QDKP2GUI_Log:ShowPlayer(W2)
        elseif #W2 > 0 then
    ...
    elseif W1 == "set" then
        ...
        if amount and W2 and (W3 == "net" or W3 == "total" or W3 == "spent" or W4 == "hours") then
            ...
            if QDKP2_IsInGuild(W2) then
                if W3 == "net" then
                    local tot, spent
                    if amount > QDKP2_GetNet(W2) then
                        tot = amount - QDKP2_GetNet(W2)
                    elseif amount < QDKP2_GetNet(W2) then
                        spent = QDKP2_GetNet(W2) - amount
                    end
    ...
    elseif W1 == "report" then
        if W2 and W3 and W4 then
            ...
            elseif W3 == "session" or W3 == "current" then
                ...
                Type = QDKP2_LOC_CurrentSession .. " " .. SessName
            ...
            local channel
            if W4 == "say" or W4 == "s" then
                channel = "SAY"
            elseif W4 == "yell" or W4 == "y" then
                channel = "YELL"
            elseif W4 == "guild" or W4 == "g" then
                channel = "GUILD"
            elseif W4 == "raid" or W4 == "r" then
                channel = "RAID"
            elseif W4 == "officer" or W4 == "o" then
                channel = "OFFICER"
            elseif W4 == "channel" then
                channel = "CHANNEL"
            elseif W4 == "whisper" or W4 == "w" then
                channel = "WHISPER"
            else
    ...
    elseif W1 == "db" or W1 == "database" then
        ...
        for i = 1, #DBguilds do
            if DBguilds[i] == "StoreVers" then
                table.remove(DBguilds, i);
            end ;
        end
        if W2 == "list" then
    ...
    elseif W1 == "debug" then
        ...
        elseif W2 == "filter" and W3 then
            ...
            if W3 == "all" then
                ...
                if status then
                    QDKP2_Msg("All debug filters active")
                else
                    QDKP2_Msg("All debug filters removed")
                end
                return
            else
                local exist = QDKP2_Debug_SetFilter(W3, status)
                if not exist then
                    QDKP2_Msg("Invalid subsystem", "ERROR");
                    return ;
                end
                if status then
                    QDKP2_Msg('Activated filter on "' .. W3 .. '" subsystem debug messages.')
                else
                    QDKP2_Msg('Removed filter on "' .. W3 .. '" subsystem debug messages.')
                end
                return
            end
        else
    ...
    elseif W1 == "makealt" then
        if W2 and W3 then
            ...
            if not QDKP2_IsInGuild(W2) then
                QDKP2_Msg(QDKP2_COLOR_RED .. W2 .. ': No matching with this guildmember name.')
            elseif not QDKP2_IsInGuild(W3) then
                QDKP2_Msg(QDKP2_COLOR_RED .. W3 .. ': No matching with this guildmember name.')
            elseif W2 == W3 then
                QDKP2_Msg(QDKP2_COLOR_RED .. "An alt's Main must be different from the alt himself.")
            elseif QDKP2_IsAlt(W3) then
    ...
    elseif W1 == "clearalt" then
        if W2 then
            ...
            if not QDKP2_IsInGuild(W2) then
                QDKP2_Msg(QDKP2_COLOR_RED .. 'No matching with that guildmember name.')
            elseif not QDKP2_IsAlt(W2) then
                QDKP2_Msg(QDKP2_COLOR_RED .. "That player is not an alt.")
            else
                QDKP2altsRestore[W2] = ""
                QDKP2alts[W2] = nil
                QDKP2_DownloadGuild(nil, false)
    ...
    elseif W1 == "external" then
        ...
        elseif W2 == "add" and W3 then
            W3 = QDKP2_FormatName(W3)
            if not QDKP2_NewExternal(W3) then
                QDKP2_Msg("Couldn't add " .. W3 .. " to the guild roster.", "ERROR");
                return ;
            end
        elseif W2 == "remove" and W3 then
            W3 = QDKP2_FormatName(W3)
            if not QDKP2_DelExternal(W3) then
                return ;
            end
        else
    ...
    elseif W1 == "standby" then
        if W2 == "list" then
            if #QDKP2standby == 0 then
                ...
                for i = 1, #QDKP2standby do
                    QDKP2_Msg(QDKP2standby[i]);
                end
            end
            return
        elseif W2 == "add" and W3 then
            W3 = QDKP2_FormatName(W3)
            if not W3 then
                QDKP2_Msg('You must specify the name of a guild member.');
                return
            elseif not QDKP2_IsInGuild(W3) then
                QDKP2_Msg('The given player is not in the Guild', "ERROR");
                return
            elseif QDKP2_IsInRaid(W3) then
                QDKP2_Msg('The given player is already in the Raid', "WARNING");
                return
            elseif QDKP2_AddStandby(W3) then
                QDKP2_Msg(W3 .. " added to the raid")
            else
                QDKP2_Msg("Couldn't add " .. W3 .. " to the raid roster.", "ERROR");
                return
            end
        elseif W2 == "remove" and W3 then
            W3 = QDKP2_FormatName(W3)
            if not W3 then
                QDKP2_Msg('You must specify the name of a guild member.');
                return
            elseif not QDKP2_IsStandby(W3) then
                QDKP2_Msg(W3 .. " is not a standby member.", "WARNING")
            elseif QDKP2_RemStandby(W3) then
                QDKP2_Msg(W3 .. " removed from the raid")
            else
                QDKP2_Msg("Couldn't remove " .. W3 .. " to the raid roster.", "ERROR");
                return
            end
        else
    ...
    elseif W1 == "classdkp" then
        if W2 and W3 then
            ...
            local channel
            if W3 == "say" or W3 == "s" then
                channel = "SAY"
            elseif W3 == "yell" or W3 == "y" then
                channel = "YELL"
            elseif W3 == "guild" or W3 == "g" then
                channel = "GUILD"
            elseif W3 == "officer" or W3 == "o" then
                channel = "OFFICER"
            elseif W3 == "channel" then
                channel = "CHANNEL"
            elseif W4 == "raid" or W4 == "r" then
                channel = "RAID"
            elseif W3 == "whisper" or W3 == "w" then
                channel = "WHISPER"
            else
...
function QDKP2_CLI_IsLegalOnOfT(word)
    if QDKP2_CLI_IsOnOff(word) == true then
        return "on"
    elseif QDKP2_CLI_IsOnOff(word) == false then
        return "off"
...
function QDKP2_CLI_IsOnOff(word)
    if word == "on" or word == "yes" or word == "y" or word == "show" or word == "1" or word == "activate" or word == "active" or word == "start" then
        return true
    elseif word == "off" or word == "no" or word == "n" or word == "hide" or word == "0" or word == "deactivate" or word == "deactive" or word == "stop" then
        return false
    end
end
