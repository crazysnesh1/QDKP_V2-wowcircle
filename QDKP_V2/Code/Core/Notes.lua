...
local function ExtractNum(str, keys)
    ...
    for j = 1, table.getn(keys) do
        ...
        if i1 and i2 then
            ...
            if (i1 == nil or i2 == nil or i2 < i1) then
                break ;
            end   --control #1
            ...
            if (i1 > 1) then
                if (string.sub(str, i1 - 1, i1 - 1) == "-") then
                    value = 0 - value;
...
function QDKP2_MakeNote(incNet, incTotal, incSpent, incHours)
    ...
    if incNet > limitMax then
        incNet = limitMax
    elseif incNet < limitMin then
        incNet = limitMin
    end
    if optValue > limitMax then
        optValue = limitMax
    elseif optValue < limitMin then
        optValue = limitMin
    end
    if incHours > 999.9 then
        incHours = '999.9'
    elseif incHours < 0 then
        incHours = '0'
    end
...
function QDKP2_FirstWord(str)
    ...
    local first = string.utf8sub(str, 1, 1)
    local fin = string.find(str, " ") or (string.utf8len(str) + 1)
    local remainder = string.utf8sub(str, 2, fin - 1)
    local output = string.utf8upper(first) .. string.lower(remainder)
