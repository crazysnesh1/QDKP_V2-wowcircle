...
function QDKP2_NotifyAll()
    if QDKP2_ManagementMode() then
        for i = 1, QDKP2_GetNumRaidMembers() do
            ...
            if online and inguild and not removed then
                QDKP2_Notify(name);
            end
        end
    else
...
function QDKP2_Debug_SetFilter(System, Status)
    ...
    local subsystems = {}
    table.foreach(QDKP2_Debug_Subsystems, function(key, val)
...
function QDKP2_minRank(name)
    ...
    for i = 1, table.getn(QDKP2_UNDKPABLE_RANK) do
        --checks the rank
...
function QDKP2_Msg(msg, Type, Color, Channel)
    ...
    elseif Channel == "CHAT" then
        --this is to avoid printing 2 times the same thing.
...
function QDKP2_UTF8len(str)
    ...
    for i = 1, #str do
        ...
        if char < 128 or (char > 193 and char < 245) then
            l = l + 1;
        end
    end
    return l
...
function ListFromDict(dict)
    ListFromDict_output = {}
    table.foreach(dict, function(key, value)
...
function QDKP2_netSort(nameList)
    --sorts a list of guildmebers by inverse net.
