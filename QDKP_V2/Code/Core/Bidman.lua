...
local function notify_winner(winner, sure)
    ...
    local GiveLoot
    if QDKP2_BidM.ITEM then
        GiveLoot = QDKP2_GiveLootToPlayer(QDKP2_BidM.ITEM, winner, true);
    end
    ...
    if QDKP2_IsInGuild(winner) and QDKP2_BidM_LogBids and QDKP2_BidM.ITEM and #QDKP2_BidM.ITEM > 0 then
        ...
        if QDKP2_IsManagingSession() then
            QDKP2log_Link("RAID", winner, timestamp, QDKP2_OngoingSession());
        end
...
function QDKP2_BidM_BidWatcher(txt, player, channel)
    ...
    if not QDKP2_BidM_isBidding() or not QDKP2_BidM.ACCEPT_BID then
        return ;
    end
    if channel == "WHISPER" and not QDKP2_BidM_GetFromWhisper then
        return ;
    end
    if channel ~= "WHISPER" and not QDKP2_BidM_GetFromGroup then
        return ;
    end
        ...
        for j, w in pairs(kwl) do
            ...
            if string.find(txt, trigStr) then
                ...
                if QDKP2_BidM_AllowMultipleBid or not (oldBet and oldBet.txt) then
                    ...
                    if not QDKP2_UpdateBid(player, newBet) then
                        return ;
                    end
                    --controls
                    ...
                    if newBet.value and oldValue then
                        if oldValue == newBet.value then
                            --is it the same bid as the previous?
                            QDKP2_BidM_SendMessage(player, "NOBID", channel, QDKP2_LOC_BidEqual)
                            return false
                        elseif oldValue > newBet.value and not QDKP2_BidM_AllowLesserBid then
                            --is the bid less then the previous?
                    ...
                    if dkp then
                        if not QDKP2_BidM_OverBid and QDKP2_GetNet(player) < newBet.minBid then
                            --has the player enought dkp to bid at all?
                            ...
                            return false
                        elseif dkp < newBet.minBid then
                            --does the bid reach the minimum bid amount?
                        ...
                        elseif newBet.maxBid and dkp > newBet.maxBid then
                            ...
                            QDKP2_BidM_SendMessage(player, "NOBID", channel, mess)
                            return false
                        elseif not QDKP2_BidM_OverBid and dkp > QDKP2_GetNet(player) then
                            --is the bid more than the player has?
...
local function EvaluateExpression(player, bid_obj, expr, bidList)
    ...
    if sectopbid and mintowin > QDKP2_BidM_MinBid and mintowin > bidList[#bidList] then
        mintowin = bidList[#bidList];
    end
    ...
    local Exec, value
    if bid_obj.asFunc then
        Exec, value = loadstring(expr)
    else
        Exec, value = loadstring("return " .. expr)
    end
    if not Exec then
        return nil, value;
    end
    ...
    if callStat and type(value) == 'number' and QDKP2_BidM_RoundValue then
...
function QDKP2_UpdateBid(player, bid_obj, finalUpdate)
    ...
    for iplayer, ibid in pairs(QDKP2_BidM.LIST) do
        if ibid.value then
            table.insert(bidList, ibid.value);
        end
    end
    table.sort(bidList)
    ...
    if not bid_voice.eligible then
        bid_obj.eligible = true
    else
...
function QDKP2_BidM_SendMessage(player, t, channel, txt)
    ...
    if channel == "bid_start" then
        channel = QDKP2_BidM_ChannelStart
    elseif channel == "bid_cancel" then
        channel = QDKP2_BidM_ChannelCanc
    elseif channel == "bid_winner" then
...
function QDKP2_BidM_RollWatch(player, roll, rollLow, rollHigh)
    ...
    if not QDKP2_BidM_isBidding() or not QDKP2_BidM_CatchRoll or not QDKP2_BidM.ACCEPT_BID then
        return ;
    end
