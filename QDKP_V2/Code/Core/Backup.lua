...
function QDKP2_Backup()
    ...
    QDKP2_Msg(QDKP2_COLOR_GREEN .. QDKP2_LOC_BK_COMPLETE .. table.getn(tempBackup) .. QDKP2_LOC_BK_ENTRIES)
...
function QDKP2_Restore(DoNotAsk)
    ...
    if not QDKP2backup.DATE then
        QDKP2_Msg(QDKP2_LOC_BK_NOT_FOUND)
        return
    end
    if not DoNotAsk then
        local mess = QDKP2_LOC_BK_RESTORE_ALL_DATA_LAST_BK
        ...
        for i = 1, table.getn(tempBackup) do
            ...
            if QDKP2_IsInGuild(name) and not QDKP2_IsAlt(name) then
                ...
                if DTotal or DSpent or DHours then
                    QDKP2_AddTotals(name, DTotal, DSpent, DHours, QDKP2_LOC_BK_RESTORED_BK, true, nil, nil, true)
                end
            elseif not QDKP2_IsAlt(name) then
                local mess = QDKP2_LOC_BK_NOT_RESTORED_BECAUSE_ALT
                mess = string.gsub(mess, "$ALT", name)
                mess = string.gsub(mess, "$MAIN", QDKP2_GetMain(name))
                QDKP2_Debug(2, "Core", mess)
            else
                QDKP2_Debug(2, "Core", name .. QDKP2_LOC_BK_NOT_RESTORED_BECAUSE_NOT_GUILD)
            end
            count = count + 1
        end
        local mess = QDKP2_LOC_BK_RESTORED_ENTRIES
        mess = string.gsub(mess, "$GET", get)
        QDKP2_Msg(QDKP2_COLOR_GREEN .. mess)
        QDKP2_RefreshGuild()
