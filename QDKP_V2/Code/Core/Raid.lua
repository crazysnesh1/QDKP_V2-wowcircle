...
local function CheckDeleted()
    local tempRemoved = {}
    table.foreach(QDKP2raidRemoved, function(key, value)
        tempRemoved[key] = true;
    end)
    for i = 1, GetNumRaidMembers() do
        local name = GetRaidRosterInfo(i)
        if tempRemoved[name] then
            tempRemoved[name] = nil;
        end
    end
...
function QDKP2_UpdateRaid()
    ...
    if QDKP2_IsRaidPresent() then
        ...
        for i = 1, QDKP2_GetNumRaidMembers() do
            ...
            if name then
                --this is to avoid exception due to a broken raid roster OR when you are in a battleground raid.
                ...
                if inguild and not removed then
                    ...
                    if Manag and not NameList[name] then
                        --wasn't in the raid, joins now
                        if standby then
        ...
        if #QDKP2raid >= 1 then
            -- if i had raid and now i have it no more means that i left (o'rly?)
            QDKP2_ILeftRaid()
            gotChanges = true
        end
    end
...
function QDKP2_IsInRaid(namePlayer)
    --returns true if the player is in the raid
    for i = 1, QDKP2_GetNumRaidMembers() do
...
function QDKP2_IsRaidPresent()
    if QDKP2_GetNumRaidMembers() > 0 then
        return true
    end
end
...
function QDKP2_AddStandby(name)
    ...
    if QDKP2_IsInRaid(name) then
        ...
        return
    elseif QDKP2_IsInGuild(name) then
