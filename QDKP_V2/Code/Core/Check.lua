...
local function CheckGo()
    ...
    for i = 1, GetNumGuildMembers(true) do
        ...
        if QDKP2_IsInGuild(name) and not QDKP2_IsAlt(name) then
            local Datafield
            if QDKP2_OfficerOrPublic == 1 then
                Datafield = officernote
            elseif QDKP2_OfficerOrPublic == 2 then
                Datafield = note
            end
...
local function CheckStart()
    if not QDKP2_CheckInProgress then
        return ;
    end
...
function QDKP2_StopCheck(doNotInform)
    ...
    if QDKP2_CheckInProgress then
        if not doNotInform then
            QDKP2_Msg(QDKP2_LOC_CheckAborted, "WARNING");
        end
