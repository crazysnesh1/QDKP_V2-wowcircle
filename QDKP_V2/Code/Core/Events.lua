...
function QDKP2_OnEvent(self, event, arg1, arg2, arg3, arg4, arg5, arg6, arg7)
    ...
    if event == "COMBAT_LOG_EVENT_UNFILTERED" then
        if arg2 == "UNIT_DIED" and QDKP2_ManagementMode() and arg7 then
            --fired when an hostile mob dies nearby
            local boss = arg7
    ...
    elseif (event == "RAID_ROSTER_UPDATE" or event == "PARTY_MEMBERS_CHANGED") then
        --fired on raid members add/leave, maybe even on loot change
        if QDKP2GUI_Roster then
            QDKP2GUI_Roster.Sort.LastLen = -1;
        end --this forces a resort.
        QDKP2_UpdateRaid()


        -- GUILD ROSTER UPDATED
    elseif (event == "GUILD_ROSTER_UPDATE") then
        --fired when a new fresh guild cache is downloaded.
        if GetNumGuildMembers(true) > 0 then
            if QDKP2_EnteredWorld and not QDKP2_ACTIVE then
                QDKP2_OnLoad();
            end
        ...
        if QDKP2processedWhispers ~= time() .. arg2 .. arg1 then
            ...
            if answer then
                ...
                if QDKP2_BidM_isBidding() then
                    ...
                    if goodBid ~= nil and QDKP2_BidM_HideWispBids then
                        --BidWatcher returns nil if it doesn't find any bid word.
                        QDKP2_SuppressIncomingWhisper(arg2, arg1)
                    end
                end
            end
    ...
    elseif event == "CHAT_MSG_RAID" or event == "CHAT_MSG_RAID_WARNING" or event == "CHAT_MSG_RAID_LEADER" or event == "CHAT_MSG_PARTY" then
        ...
        if QDKP2_BidM_isBidding() then
            QDKP2_BidM_BidWatcher(arg1, arg2, channel);
        end
    ...
    if not QDKP2_ACTIVE or not QDKP2_ManagementMode() then
        return ;
    end
