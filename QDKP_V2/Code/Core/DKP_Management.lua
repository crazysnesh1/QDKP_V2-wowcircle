...
function QDKP2_RaidAward(amount, reason, giverName)
    --if giverName != nil, this will become a zerosum share.
    ...
    if type(amount) == "string" then
        if giverName then
            amount = QDKP2_GetAmountFromText(amount, giverName)
        else
    ...
    for i = 1, QDKP2_GetNumRaidMembers() do
        ...
        if inguild and not removed and not QDKP2_IsMainAlreadyProcessed(name) then
            ...
            if eligible then
                ...
                if NowHours ~= OrigHours then
                    --use this to detect if i've hit an integer (eg 2.9 + 0.2 = 3.1)
...
function QDKP2_CheckHours()
    ...
    if QDKP2_isTimerOn() then
        ...
        elseif not QDKP2_IsManagingSession() then
            --if i leave the raid/stop the session, close the timer.
            QDKP2_TimerOff()
        end
    end
end

function QDKP2_TimerOn(DoNotControl)
    -- start the timer
...
function QDKP2_IronManStart()
    --sets the ironman mark
...
function QDKP2_IronManWipe()
    --abort the ironman couting without giving anything
...
function QDKP2_InronManFinish(BonusDKP)
    --calculates who award the ironman bonus and award them BonusDKP
    ...
    for i = 1, #playerList do
        ...
        if eligible and isIn and wasIn then
            ...
            for j = #LogList, 0, -1 do
                ...
                local Log = LogList[j]
                if Log then
                    LogType, LogTime = QDKP2log_GetData(Log)
                else
                    --trick to close the calculation after the last log entry.
                    LogTime = endMark
                    LogType = QDKP2LOG_LEFT
                end
                if LogType == QDKP2LOG_JOINED and not online then
                    if LogTime > timePos then
                        timePos = LogTime;
                    end
                    online = true
                elseif LogType == QDKP2LOG_LEFT and online then
                    if LogTime > timePos then
                        timeTot = timeTot + LogTime - timePos;
                    end
                    online = false
                end
            end
...
function QDKP2_DetectBidSet(todo)
    ...
    elseif todo == "on" then
        ...
        QDKP2_Msg(QDKP2_COLOR_YELLOW .. QDKP2_LOC_GUIWINNERDETECTON)
    elseif todo == "off" then
        QDKP2_DetectBids = false
        QDKP2_Events:Fire("DETECTBID_OFF")
        QDKP2_Msg(QDKP2_COLOR_YELLOW .. QDKP2_LOC_GUIWINNERDETECTOFF)
    end
end
...
function QDKP2_GetEligibility(name, awardtype, award, online, inzone)
    ...
    if QDKP2_IsExternal(name) then
        ...
        if perc then
            percentage = perc;
            reason = QDKP2LOG_NODKP_EXTERNAL;
        end
    end
