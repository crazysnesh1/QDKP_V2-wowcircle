...
function QDKP2_IsActiveDKPEntry(Type)
    if Type == QDKP2LOG_MODIFY or Type == QDKP2LOG_CONFIRMED or Type == QDKP2LOG_SESSION or Type == QDKP2LOG_EXTERNAL then
        return true;
    end
end
...
function QDKP2_IsLinkDKPEntry(Log)
    ...
    if not QDKP2_IsDKPEntry(Type) and not QDKP2_IsNODKPEntry(Type) then
        return ;
    end
    if (Bit0 or Bit1) and not Bit2 then
        -- if RaidAward or ZeroSum but not main entry
        return true
    end
end
...
function QDKP2_IsVersionedEntry(Type)
    if QDKP2_IsDKPEntry(Type) or QDKP2_IsNODKPEntry(Type) or Type == QDKP2LOG_LOOT then
        return true;
    end
end
...
function QDKP2_IsSessionRelated(Type)
    if Type == QDKP2LOG_JOINED or Type == QDKP2LOG_LEFT or Type == QDKP2LOG_LOOT or Type == QDKP2LOG_BOSS or Type == QDKP2LOG_NODKP or Type == QDKP2LOG_BIDDING then
        return true;
    end
end

function QDKP2_Timestamp()
    --returns a timestamp with a Entry ID, to avoid collisions on linked items
