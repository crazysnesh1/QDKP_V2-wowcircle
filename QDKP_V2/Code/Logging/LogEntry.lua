...
function QDKP2log_Entry(name, action, Type, undo, timestamp, flags, outsideSession)
    ...
    if Type == QDKP2LOG_MODIFY then
        QDKP2log_BackupSave(SID, name, tempEntry, true);
    end
    ...
    if name == "RAID" then
        MaxSize = QDKP2_LOG_RAIDMAXSIZE;
    end
    MaxSize = MaxSize - 1
    if LogSize > MaxSize then
        LogSize = MaxSize;
    end  --pop the last entries if i'm at the maximum log's length
    for buildLog = 1, LogSize do
        table.insert(tempLog, PlayersLog[buildLog]);
    end
...
function QDKP2log_BackupSave(SID, name, Log, deleteIt)
    ...
    local key = SID .. "|" .. ts
    if not playerBackups[key] then
        --if not nil then i already have the original saved.
        ...
        if Undo then
            backupLog[QDKP2LOG_FIELD_AMOUNTS] = QDKP2_CopyTable(Undo);
        end
        if deleteIt then
            backupLog[QDKP2LOG_FIELD_TYPE] = nil;
        end
...
function QDKP2log_GetModEntryText(Log, isRaid)
    ...
    elseif QDKP2_IsDKPEntry(Type) then
        ...
        elseif reason then
            ...
            elseif spent and not gained and not hours and GetItemIcon(reason) then
                --i use getitemicon to tell if reason is a valid item name/link
    ...
    elseif QDKP2_IsNODKPEntry(Type) then
        ...
        if gained then
            ...
            if Mod and Mod ~= 100 then
                gainTxt = gainTxt .. "x" .. tostring(Mod) .. "%%";
            end
...
function QDKP2log_GetAmounts(Log)
    ...
    local gain
    if Ngain then
        gain = RoundNum(Ngain * (Coeff or 100) / 100);
    end
...
function QDKP2log_GetChange(LogEntry)
    ...
    if not QDKP2_IsActiveDKPEntry(Type) then
        return ;
    end
...
function QDKP2log_UnDoEntry(name, Log, SID, onofftog)
    ...
    if onofftog == "on" and LogType ~= QDKP2LOG_ABORTED then
        return
    elseif onofftog == "off" and LogType == QDKP2LOG_ABORTED then
        return
    elseif LogType == QDKP2LOG_ABORTED then
        onofftog = "on"
    else
        onofftog = "off"
    end
...
function QDKP2log_SetEntry(name, Log, SID, newGained, newSpent, newHours, newCoeff, newReason, Activate, NoIncreaseVersion)
    ...
    local RaidAw, ZeroSum, MainEntry = QDKP2log_GetFlags(Log)
    if MainEntry and not NoProcessZs then
        --update linked dkp entries
...
function QDKP2log_AwardLinkedEntry(name, Log, SID, todo, ReasonCode)
    ...
    elseif todo == 'not-award' and not QDKP2_IsNODKPEntry(Type) then
        ...
        if not V0 then
            ReasonCode = QDKP2LOG_NODKP_MANUAL;
        end --default reason for entries that are switched off (Manually removed)
        if ReasonCode then
            QDKP2log_SetFlags(Log, RaidAw, ZeroSum, MainEntry, B3, ReasonCode, V1)
        end
    elseif todo == 'toggle' then
        if QDKP2_IsNODKPEntry(Type) then
            QDKP2log_AwardLinkedEntry(name, Log, SID, 'award', ReasonCode)
        else
            QDKP2log_AwardLinkedEntry(name, Log, SID, 'not-award', ReasonCode)
        end
    else
...
function QDKP2log_UpdateLinkedDKPLog(name, Log, SID, NoIncreaseVersion)
    ...
    for i = 1, #nameList do
        ...
        if not QDKP2_IsAlt(name2) and name2 ~= name then
            ...
            local todo
            if LogType == log2type then
                todo = nil --all ok
            elseif QDKP2_IsActiveDKPEntry(LogType) and not QDKP2_IsActiveDKPEntry(log2type) then
                todo = 'on'
            elseif not QDKP2_IsActiveDKPEntry(LogType) and QDKP2_IsActiveDKPEntry(log2type) then
                todo = 'off'
            end
            if todo then
                QDKP2log_UnDoEntry(name2, log2, SID, todo);
            end
        end
    end
    return true
...
function QDKP2log_SetAmountsField(Log, newValue, NoIncreaseVersion)
    ...
    if newValue[3] == 0 then
        newValue[3] = nil;
    end
    if newValue[4] == 100 then
        newValue[4] = nil;
    end
...
function QDKP2_ZeroSum_Update(giverName, mainLog, SID, NoIncreaseVersion)
    ...
    if Sharer > 0 then
        ...
        while true do
            ...
            elseif oldTotal == Total then
                --this is to avoid division by zero, but should never happen.
                --pass
            else
