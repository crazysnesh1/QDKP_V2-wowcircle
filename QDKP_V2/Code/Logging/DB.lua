...
function QDKP2log_Find(LogList, timestamp)
    ...
    if not timestamp then
        return { QDKP2LOG_INVALID, 0, "*FIND LOG: No timestamp in the query*" };
    end
    if not LogList or table.getn(LogList) <= 0 then
        return { QDKP2LOG_INVALID, timestamp, "*FIND LOG: Null or empty Log List passed*" };
    end
    local index = QDKP2log_FindIndex(LogList, timestamp)
    if not index then
        return { QDKP2LOG_INVALID, timestamp, "*FIND LOG: Can't find the entry*" };
    end
...
function QDKP2log_FindLink(LinkLog)
    ...
    if not timestamp then
        return { QDKP2LOG_INVALID, nil, QDKP2_LOC_InvalidLinkTime };
    end
    if not name then
        return { QDKP2LOG_INVALID, timestamp, QDKP2_LOC_InvalidLinkPlayer };
    end
    if not SID then
        return { QDKP2LOG_INVALID, timestamp, QDKP2_LOC_InvalidLinkSession };
    end
    if not QDKP2log[SID] then
        return { QDKP2LOG_INVALID, timestamp, QDKP2_LOC_InvalidLinkSessName };
    end
    local LogList = QDKP2log[SID][name]
    if not LogList or table.getn(LogList) <= 0 then
        return { QDKP2LOG_INVALID, timestamp, "*INVALID LINK: Can't find linked player (" .. name .. ")'s log*" };
    end
    local index = QDKP2log_FindIndex(LogList, timestamp)
    if not index then
        return { QDKP2LOG_INVALID, timestamp, "*INVALID LINK: Can't find the entry in linked player's log (" .. name .. ")*" };
    end
...
function QDKP2log_ParsePlayerLog(Name)
    ...
    --      ...
    if not Name then
        error("QDKP2log_ParsePlayerLog: <Name> parameter is nil!");
    end
    ...
    for i = 1, table.getn(PlayerSessions) do
        ...
        if SID ~= "0" then
            ...
            local UnDo
            if Name == "RAID" then
                local GainDKP, SpentDKP, Hours = QDKP2log_SessionStatistics(SID);
                UnDo = { GainDKP, SpentDKP, Hours }
            else
                UnDo = QDKP2log_ResumDKPChanges(SessionLogList)
            end
    ...
    if PlayerGeneralSess then
        for i = 1, #PlayerGeneralSess do
            table.insert(out, PlayerGeneralSess[i]);
        end
    end
...
function QDKP2log_PurgeSessions(number, Sure)
    ...
    if table.getn(LogList) <= number then
        return ;
    end
...
function QDKP2log_CreatedDateSort(log1, log2)
    ...
    if log1[QDKP2LOG_FIELD_TIME] > log2[QDKP2LOG_FIELD_TIME] then
        return true
    else
        return false
    end
