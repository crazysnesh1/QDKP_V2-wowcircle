...
local Channels = { GUILD = L.AN_AnChannel1, RAID = L.AN_AnChannel2, PARTY = L.AN_AnChannel3, GROUP = L.AN_AnChannel9,
                   RAID_WARNING = L.AN_AnChannel7, OFFICER = L.AN_AnChannel4, BATTLEGROUND = L.AN_AnChannel8,
                   SAY = L.AN_AnChannel5, YELL = L.AN_AnChannel6 }
local ChannelsPers = { GUILD = L.AN_AnChannel1, RAID = L.AN_AnChannel2, PARTY = L.AN_AnChannel3, GROUP = L.AN_AnChannel9,
                       RAID_WARNING = L.AN_AnChannel7, OFFICER = L.AN_AnChannel4, BATTLEGROUND = L.AN_AnChannel8,
                       SAY = L.AN_AnChannel5, YELL = L.AN_AnChannel6, WHISPER = L.AN_AnChannel10, BID = L.AN_AnChannel11 }
...
local function TestKey(info)
    ...
    for i, v in pairs(voice) do
        if v and i ~= 'keywords' and v ~= '' then
            ...
            --	v=string.gsub(v,"$itemprice",)
            v = string.gsub(v, "$item", 'Axe of Uberness')
            v = string.gsub(v, "$n", '80')
            local Exec, value
            if string.sub(v, 1, 9) == 'function ' then
                Exec, value = loadstring(string.sub(v, 10))
            else
                Exec, value = loadstring("return " .. v)
            end
            if not Exec then
    ...
    args = {
        BidOptions = {
            ...
            args = {
                ...
                BM_AnnounceStartStep = {
                    type = 'toggle',
                    order = 1031,
                },
                BM_AnnounceStartStepChannel = {
                    type = 'select',
                    values = Channels,
                    order = 1032,
                    style = 'dropdown',
            ...
            args = {
                DefaultKWProfile = {
                    ...
                    confirm = function(info)
                        if GetPreset(info) == "CUSTOM" then
                            return L.DefaultKWProfile_c;
                        else
                            return false;
                        end ;
                    end,
                ...
                ProfileDesc = {
                    type = 'description',
                    name = function(info)
                        local i = GetPreset(info);
                        return QDKP2_Config.KeywordPresets[i].description;
                    end,
                    order = 20,
                },
                AddNewKeyword = {
...
QDKP2_Config.KeywordPresets = {
    ...
    A = {
        name = 'Simple Bidding',
        description = 'A simple bidding system. Every guild player can place bets regardless of rank, class or anything, and the winner is charged by the plain bet amount (if any).This bidding system can also be used when loot council is used, as it will import "need" and "greed" words as 0 dkp bets.\
To place bets, players have simply to say a number ("bid #" and "need #" will work, too). Players can place default bets by saying "all" or "all in", and that will place all the DKP they have, "half" that will place half that amount, and "min" will place the minimum set bid amount.',
    ...
    B = {
        name = 'Spend one more ',
        description = 'In this bidding system the winner is not necessarly charged for the full amount he bet, but only for the strict amount needed to win, and that is the amount bet by the second highest bidder plus 1, or the minimum bid if no one else is bidding.\
...
The keywords used in this template are the same used in the simple bidding. To place bets, players have simply to say a number ("bid #" and "need #" will work, too)  Players can place default bets by saying "all" or "all in", and that will place all the DKP they have, "half" that will place half that amount, and "min" will place the minimum set bid amount.',
    ...
    C = {
        name = 'Fixed steps',
        description = 'Every bet made in this bidding system must be greater than the curret highest one by a fixed amount. It can speedup bid process whenever you have multiple public bets and your members increase their bet one by one to the point it becomes tedious.\
...
',
    ...
    F = {
        name = 'Roll+Net',
        description = "The roll+net bidding system is a simple system where the bet value is calculated as the sum between net DKP and a roll. Has the advantage to be quick and easy like a roll but still advantage the players who raid more.\
...
Please note that the Ni Karma system requests a maximum net DKP cap of 100',
    ...
    H = {
        ...
        description = 'In this bidding system set fixed prices are used as bid starting price. The keywords used are the same as in the simple bidding, with the difference that /rolls and "needs" will cost DKP, too.',
        keys = {
    ...
    K = {
        ...
        description = 'This is a bidding system with complex keywords. It s only for reference to show various advanced aspects of the Quick DKP bid manager and should not used as it is by anyone.\
    ...
    CUSTOM = {
        name = 'Custom system',
        description = 'The current looting system has been customized.\
