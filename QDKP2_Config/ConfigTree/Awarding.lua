...
local function GetBossVoice(info)
    local i = info[#info - 1]
    local field
    if string.find(info[#info], '10') then
        field = 'DKP_10'
    else
        field = 'DKP_25'
    end
...
local function GetAmountName(info)
    ...
    if t == 'Inst10' then
        return L["Instance10"]
    elseif t == 'Inst10H' then
        return L["Instance10H"]
    elseif t == 'Inst25' then
        return L["Instance25"]
    elseif t == 'Inst25H' then
        return L["Instance25H"]
    end
end

local function GetHidden(info)
    if GetInstName(info) == '' then
        return true;
    end
end
...
local function GetColName(info)
    ...
    local col
    if v == 0 then
        col = QDKP2_Config.Colors.Disabled
    elseif v == 1 then
        col = QDKP2_Config.Colors.Enabled
    else
        col = ''
    end
...
local BossVoice = {
    ...
    args = {
        ...
        Select10 = {
            type = 'select',
            values = { L.AW_Select1, L.AW_Select2, L.AW_Select3 },
...
for i = 1, 99 do
    --maximum of 99 boss names entries. Seems resonable to me.
...
for i = 1, 20 do
    --maximum of 20 instances entries.
