...
local function IsSession(Log)
    if QDKP2log_GetType(Log) == QDKP2LOG_SESSION then
        return true;
    end
end
...
myClass.MakeReport = function(list, Type, channel, subChannel)
    ...
    local Add = {}
    if Type == "this" then
        Header = QDKP2_LOC_SingleLogVoice
    elseif Type == "sub" then
        ...
        Header = QDKP2_LOC_Session .. " " .. Name
        if DateStop then
            table.insert(Add, QDKP2_LOC_ClosedOn .. QDKP2_GetDateTextFromTS(DateStart));
        end
        if DateStart then
            table.insert(Add, QDKP2_LOC_StartedOn .. QDKP2_GetDateTextFromTS(DateStart));
        end
        table.insert(Add, QDKP2_LOC_SessionManager .. tostring(Mantainer or '-'))
    elseif Type == "curr" then
        Header = QDKP2_LOC_PersonalOverview
    end
    local logList = {}
