...
function myClass.Refresh(self, doNotUpdate)
    if not myClass.Frame:IsVisible() then
        return ;
    end
    ...
    if myClass.Offset > #myClass.List then
        myClass.Offset = #myClass.List - 1;
    end
    if myClass.Offset < 0 then
        myClass.Offset = 0;
    end
    ...
    for i = 1, myClass.ENTRIES do
        ...
        if (indexAt <= #myClass.List) then
            ...
            if Voice.DKPhist then
                getglobal(ParentName .. "_net"):SetText(Voice.DKPhist);
            else
                getglobal(ParentName .. "_net"):SetText("")
            end
            if Voice.DKPchange then
                getglobal(ParentName .. "_mod"):SetText(Voice.DKPchange);
            else
                getglobal(ParentName .. "_mod"):SetText("");
            end
            ...
            if logType == QDKP2LOG_LINK then
                --the GetType function is not trasparent to links, so I have to explicit the case.
            ...
            if Voice.DKPhist and Voice.DKPhist < 0 then
                netColors = QDKP2log_GetEntryColor("NegativeDKP")
            elseif Voice.DKPhist and Voice.DKPhist > 0 then
                netColors = QDKP2log_GetEntryColor("PositiveDKP")
            end
            if QDKP2_IsDeletedEntry(Voice.Type) then
                amountColors = QDKP2log_GetEntryColor("LostDKP")
            elseif Voice.DKPchange and Voice.DKPchange < 0 then
                amountColors = QDKP2log_GetEntryColor("NegativeDKP")
            elseif Voice.DKPchange and Voice.DKPchange > 0 then
                amountColors = QDKP2log_GetEntryColor("PositiveDKP")
            end
            ...
            if Voice.SubList then
                expandeBtn:Show()
                if myClass.Expanded[Voice.ID] then
                    expandeBtn:SetNormalTexture("Interface\\Buttons\\UI-MinusButton-Up")
                else
                    expandeBtn:SetNormalTexture("Interface\\Buttons\\UI-PlusButton-Up")
                end
            else
                expandeBtn:Hide()
            end
            getglobal(ParentName):Show();
            if myClass.Selected == Voice.ID then
    ...
    if #myClass.List < numEntries then
        numEntries = #myClass.List;
    end
...
function myClass.UpdateExpanded(self)
    ...
    for i = 1, #myClass.ServiceList do
        ...
        if v.SubList and myClass.Expanded[v.ID] then
            for j = 1, #v.SubList do
                table.insert(list, v.SubList[j]);
            end
        end
    end
    myClass.List = list
...
function myClass.ExpandeToggle(self, Voice)
    ...
    local ID = Voice.ID
    if myClass.Expanded[ID] then
        myClass.Expanded[ID] = false
    else
        myClass.Expanded[ID] = true
    end
    myClass:UpdateExpanded()
...
function myClass.ExpandeAll()
    for i, Voice in pairs(myClass.ServiceList) do
        if Voice.SubList then
            myClass.Expanded[Voice.ID] = true;
        end
    end
    myClass:UpdateExpanded()
    myClass:Refresh()
...
function myClass.GoTo(self, Type, MainSelection, SubListID, ID, resetHistory)
    ...
    if myClass.Offset <= myClass.ENTRIES then
        myClass.Offset = 0;
    end
...
local function isTargetGoodReport()
    if UnitName("target") and UnitIsPlayer("target") and UnitIsFriend("player", "target") then
        return true;
    end
end
...
function myClass.Report(self, Type, channel, channelSub)
    ...
    if channel == "WHISPEROWNER" then
        ...
        if not QDKP2online[channelSub] then
            return ;
        end
    elseif channel == "WHISPERTARGET" then
        if not isTargetGoodReport() then
            return ;
        end
        channel = "WHISPER"
...
local ChannelMenu = {
    { text = QDKP2_LOC_LogGuild, func = function()
        myClass:Report(UIDROPDOWNMENU_MENU_VALUE, "GUILD");
    end },
    { text = QDKP2_LOC_LogOfficer, func = function()
        myClass:Report(UIDROPDOWNMENU_MENU_VALUE, "OFFICER");
    end },
    { text = QDKP2_LOC_LogRaid, func = function()
        myClass:Report(UIDROPDOWNMENU_MENU_VALUE, "RAID");
    end },
    { text = QDKP2_LOC_LogParty, func = function()
        myClass:Report(UIDROPDOWNMENU_MENU_VALUE, "PARTY");
    end },
    { text = QDKP2_LOC_LogSay, func = function()
        myClass:Report(UIDROPDOWNMENU_MENU_VALUE, "SAY");
    end },
    { text = QDKP2_LOC_LogYell, func = function()
        myClass:Report(UIDROPDOWNMENU_MENU_VALUE, "YELL");
    end },
    { func = function()
        myClass:Report(UIDROPDOWNMENU_MENU_VALUE, "WHISPEROWNER");
    end },
    { text = QDKP2_LOC_LogWhisperTarget, func = function()
        myClass:Report(UIDROPDOWNMENU_MENU_VALUE, "WHISPERTARGET");
    end },
    { text = QDKP2_LOC_LogWhisperTo, func = function()
        myClass:Report(UIDROPDOWNMENU_MENU_VALUE, "WHISPER");
    end },
    { text = QDKP2_LOC_LogChannel, func = function()
        myClass:Report(UIDROPDOWNMENU_MENU_VALUE, "CHANNEL");
    end },
}

local ReportMenu = {
    { text = QDKP2_LOC_ThisEntry,
      hasArrow = true,
      value = "this",
      menuList = ChannelMenu,
    },
    { text = QDKP2_LOC_Subsection,
      hasArrow = true,
      value = "sub",
      menuList = ChannelMenu,
    },
    { text = QDKP2_LOC_CurrentWindow,
      hasArrow = true,
      value = "curr",
      menuList = ChannelMenu,
    },
...
local function NYIfunc()
    QDKP2_Msg(QDKP2_LOC_ToBeDone)
end

local LogVoices = {

    DisenchantedLoot = { text = QDKP2_LOC_DisenchantedLoot,
                         checked = function()
    ...
    ChargePlayer = { text = QDKP2_LOC_ChargeLooter,
                     func = function()
    ...
    ModifyEntry = { text = QDKP2_LOC_EditDKPAmounts,
                    func = function()
    ...
    EnableEntry = { text = QDKP2_LOC_EnableDKPEntry,
                    checked = function()
                        if QDKP2_IsActiveDKPEntry(myClass.MenuVoice.Type) then
                            return true;
                        end
                    end,
                    func = function()
                        local SID = '0'
                        if myClass.MenuVoice.isChild then
                            SID = myClass.MenuVoice.isChild.SubListID;
                        end
    ...
    EditMain = { text = QDKP2_LOC_EditMainEntry,
                 func = function()
                     local SID = (myClass.MenuVoice.isChild or {}).SubListID or '0'
                     ...
                     if vLog then
                         ...
                         QDKP2_Msg(QDKP2_LOC_NotFind, "ERROR")
                     end
                 end },

    SetAwRatio = { text = QDKP2_LOC_SetAwardRatio,
                   func = function()
                       local _, _, _, oldCoeff = QDKP2log_GetAmounts(myClass.MenuVoice.Log)
                       local txt = QDKP2_LOC_InsertPerc1 .. tostring(myClass.MenuVoice.Player) .. QDKP2_LOC_InsertPerc2
                       QDKP2_OpenInputBox(txt, function(newCoeff, Voice)
                           if not newCoeff or newCoeff == '' then
                               return ;
                           end
                           ...
                           if not newCoeff or newCoeff < 0 or newCoeff > 250 then
                               QDKP2_Msg(QDKP2_LOC_ValidPerc, "ERROR")
                               return
                           end
    ...
    IncludePlayer = { text = QDKP2_LOC_AwardPlayer,
                      checked = function()
                          if not QDKP2_IsNODKPEntry(myClass.MenuVoice.Type) then
                              return true;
                          end
                      end,
                      func = function()
    ...
    ViewSession = { text = QDKP2_LOC_ViewSessionLog,
                    func = function()
    ...
    ViewLinked = { text = QDKP2_LOC_ViewPlayerLog,
                   func = function()
                       local Type = 'player'
                       local Player = myClass.MenuVoice.Player
                       local SID = (myClass.MenuVoice.isChild or {}).SubListID
                       if not Player or not SID then
                           return
                       elseif Player == 'RAID' then
                           Type = 'raid';
                           Player = nil
                       end
    ...
    ChangeSessName = { text = QDKP2_LOC_ChangeSessionName,
                       func = function()
                           local SID = myClass.MenuVoice.SubListID
                           ...
                           if Name == QDKP2_LOC_NoSessName then
                               Name = '';
                           end
                           QDKP2_OpenInputBox(QDKP2_LOC_NewNameSessionText, function(NewSessionName, SID)
    ...
    ExportSession = { text = QDKP2_LOC_ExportSessionRol,
                      func = function()
                          local SID = myClass.MenuVoice.SubListID
    ...
    ExportExtSession = { text = QDKP2_LOC_ExportSessionDet,
                         func = function()
                             local SID = myClass.MenuVoice.SubListID
    ...
    ExportCurrView = { text = QDKP2_LOC_ExportLog,
                       func = function()
                           local LogList = {}
    ...
    ClearSel = { text = QDKP2_LOC_ClearSelection,
                 func = function()
                     myClass.SelectVoice(nil);
                 end,
    },

    Expand = { text = QDKP2_LOC_ExpandVoice,
               checked = function()
                   if myClass.Expanded[myClass.MenuVoice.ID] then
                       return true;
                   end ;
               end,
               dclick = true,
               func = function()
                   myClass:ExpandeToggle(myClass.MenuVoice);
               end,
    },

    ShowInfo = { text = QDKP2_LOC_ShowEntryInfo,
                 func = function()
    ...
    Report = { text = QDKP2_LOC_Report, menuList = ReportMenu, hasArrow = true },
    CollapseAll = { text = QDKP2_LOC_CollapseAll, func = myClass.CollapseAll },
    ExpandeAll = { text = QDKP2_LOC_ExpandeAll, func = myClass.ExpandeAll },
    LogUpdate = { text = QDKP2_LOC_LogUpdate, func = myClass.Refresh },
    MenuClose = { text = QDKP2_LOC_MenuClose, func = QDKP2GUI_CloseMenus },
...
function myClass.VoiceMenu(self, Voice, exe)
    ...
    table.insert(menu, 1, { text = QDKP2_LOC_MenuHeader, isTitle = true })
    ...
    end --if I have anything else than the title then space them
    ...
    for i, v in pairs(menu) do
        if v.dclick then
            ...
            if exe then
                ...
                dclick.colorCode = "|cFFBBFFBB";
                break  --this will color the entry that is going to be executed on double click.
            end
        end
    end
    ...
    if isTargetGoodReport() then
        ChannelMenu[8].disabled = false
    else
        ChannelMenu[8].disabled = true
    end
    if QDKP2_IsInGuild(Voice.Player or myClass.MainSelection) then
        ChannelMenu[7].text = string.gsub(QDKP2_LOC_WhisperTo, "$NAME", Voice.Player or myClass.MainSelection)
        ChannelMenu[7].disabled = false
    else
        ChannelMenu[7].text = QDKP2_LOC_WhisperToLogOwner
        ChannelMenu[7].disabled = true
    end
    EasyMenu(menu, myClass.MenuFrame, "cursor", 0, 0, "MENU")
    if dclick then
        dclick.colorCode = nil;
    end
end
...
function myClass.WindowMenu(self)
    ...
    table.insert(menu, { text = QDKP2_LOC_LogWindowMenu, isTitle = true })
