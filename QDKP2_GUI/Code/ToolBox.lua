...
function Toolbox.Refresh(self)
    if not Toolbox.Frame:IsVisible() and not SetAmounts.Frame:IsVisible() then
        return ;
    end
    Toolbox.EditPlayers = Toolbox.EditPlayers or {}
    if #Toolbox.EditPlayers < 1 then
        return ;
    end
...
function SetAmounts.Set(self, sure)
    ...
    if total == "" then
        ...
        total = tonumber(setTotal);
    end
    if hours == "" then
        hours = nil;
    else
        hours = tonumber(setHours);
    end
    if not net and not total and not hours then
        return ;
    end
    ...
    for i, v in pairs(Toolbox.EditPlayers) do
        if not QDKP2_ProcessedMain(v) then
            ...
            if total then
                DTotal = total - QDKP2_GetTotal(v);
            end
            if net then
                DSpent = (total or QDKP2_GetTotal(v)) - net - QDKP2_GetSpent(v);
            end
            if hours then
                DHours = hours - QDKP2_GetHours(v);
            end
            if DTotal == 0 then
                DTotal = nil;
            end
