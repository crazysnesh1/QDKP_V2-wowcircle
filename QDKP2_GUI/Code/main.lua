...
function myClass.Refresh(self)
    ...
    elseif not Session then
        QDKP2frame1_CurrentSession:SetText(QDKP2_LOC_GUINOONGOINGSESS)
    else
        local _, Name = QDKP2_GetSessionInfo(Session)
        QDKP2frame1_CurrentSession:SetText(QDKP2_COLOR_WHITE .. tostring(Name))
    end
    if not Session and QDKP2_IsRaidPresent() and QDKP2_OfficerMode() then
        QDKP2frame1_newSession:Enable()
    else
        QDKP2frame1_newSession:Disable()
    end
    if Session then
        QDKP2frame1_closeSession:Enable()
    else
        QDKP2frame1_closeSession:Disable()
    end
    ...
    if QDKP2_isTimerPaused() then
        QDKP2frame1_onOff:SetText(QDKP2_LOC_Resume)
    elseif QDKP2_isTimerOn() then
        QDKP2frame1_onOff:SetText(QDKP2_LOC_Stop)
    else
    ...
    if QDKP2_FixedPrices then
        QDKP2frame1_FixedPrice:SetChecked(1)
    else
        QDKP2frame1_FixedPrice:SetChecked(0)
    end
    ...
    if not QDKP2_IronManIsOn() then
        ...
        if BonusDKP == 0 then
            if not Sure then
                ...
                QDKP2_AskUser(mess, function()
                    myClass:Ironman(true);
                end)
                return
            end
            QDKP2_IronManWipe()
...
function myClass.TimerCountdownRefresh(self, event, timetogo, timenow)
    ...
    local paused = ""
    if QDKP2_isTimerPaused() then
        if self.TimerPauseColor then
            self.TimerPauseColor = nil
        else
            color = QDKP2_COLOR_RED;
            self.TimerPauseColor = true;
        end
    end
    if QDKP2_isTimerOn() then
